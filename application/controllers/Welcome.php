...
class Welcome extends CI_Controller {
	...
	public function doctors()
	{
		...
		}else if($this->input->post('hos_submit')){
			
			//$data['doctors'] = $this->db->get('doctors')->result();
			$data['hospital'] = $this->db->get('hospital')->result();
			
			$data['hos_active']='active';
			$data['total'] = count($data['hospital']);
			$data['gallery']=$this->db->query("SELECT hos_gallery.*,hospital.name as doctor from hos_gallery
		JOIN hospital on hospital.id=hos_gallery.hospital_id WHERE hospital.id=hos_gallery.hospital_id")->result();
			//echo $this->db->last_query();die;
		}
		else
		{
	...
	public function search()
	{
		...
		else if($this->input->post('hos_submit') && !empty($this->input->post('searched_item'))){
			
			$data['hos_active']="active";
			$data['hospital'] = $this->db->query('SELECT hospital.*,specialities.service_name,specialities.id as special_id from hospital
												join hospital_specialities_id on hospital.id=hospital_specialities_id.hospital_id
												join specialities on specialities.id=hospital_specialities_id.hospital_specialities
												where specialities.id='.$this->input->post('searched_item').'')->result();
			$data['special']=$this->db->select('service_name, id as special_id')->get('specialities')->result();
			$data['seach_item']= $this->input->post('searched_item');
			$data['total']= count($data['hospital']);
			
		}
		else if($this->input->post('doc_submit') && !empty($this->input->post('searched_item')))
		{
