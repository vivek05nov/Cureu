...
class Doctor_model extends CI_Model 
{
    ...
    function get_upcoming_appointment($id)
    {
		$today = date('Y-m-d');
		 ...
		 $toda=date("Y-m-d", strtotime($today));
        $this->db->select('a.id as id,a.member_id as member, a.status as status, m.name as customer,m.mobile_no as mobile, m.email as email, m.address as address, d.name as doctor, s.service_name as specialities,  a.appointment_date as appointment_date, a.appointment_time as appointment_time')
         ->from('appointment a')
   ...
   {   
	...
	{ 
		 ...
		 $data=array('patient_id'=>$this->input->post('hidden_member'),
					'doc_id'=>$this->input->post('hidden_doc'),
					'hospital_name'=>$this->input->post('refer_to'),
					'problem'=>$this->input->post('about_patient'),
					'created_at'=>date('Y-m-d h:i:s'),
					'status'=>1
					); 
		$this->db->insert('refer_to',$data);
			return true;
		 }
		 else
		 {
