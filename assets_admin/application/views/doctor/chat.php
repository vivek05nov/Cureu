										...
										<div class="media">
											<div class="media-img-wrap">
												<div class="avatar avatar-online">
													<img id="user_img" src="https://ptetutorials.com/images/user-profile.png" alt="User Image" class="avatar-img rounded-circle" >
												</div>
											</div>
											<div class="media-body">
												<div class="user-name">  <span id="user_name"></span>  <?php  echo  '<span id="sender_id" style="display:none;"></span>'; ?> </div>
        ...
        function chat_members(){
            ...
            $.ajax({
               url:'<?php echo base_url("inbox/admin_chat_members") ?>',
               type:'post',
        ...
        function chat(id){
         //   alert(id);
            ...
            $.ajax({
               url:'<?php echo base_url("inbox/admin_chat") ?>',
               type:'post',
               data: {sender_id:id},
               dataType:'json',
               success:function(data){
        ...
        $(document).ready(function(){
            chat_members();
			var id;
            $('#getchat').html('<div><h3>Welcome Doctor..<h3></div>');
            $(document).off('click','.chat_list').on('click','.chat_list', function(){
                ...
                $('#sender_id').text(id);
                chat(id);    
                setInterval(function(){ chat(id); }, 6000);         
            });
            ...
            $('#send_message').click(function (){
              ...
              receiver_id = $('#sender_id').text();
                if($('#sender_id').text() == ''){
                    $('#getchat').html('<div><h4 class="alert alert-danger p-2">Please select any Patient then send Message...<h4></div>');
